...
Quaternion Quaternion::Euler(double radX, double radY, double radZ)
{
	...
	radX = Utility::RadIn2PI((float)radX);
	radY = Utility::RadIn2PI((float)radY);
	radZ = Utility::RadIn2PI((float)radZ);
...
Quaternion Quaternion::FromToRotation(VECTOR fromDir, VECTOR toDir)
{
	...
	return Quaternion::AngleAxis(Utility::Deg2RadD((float)angle), axis);
