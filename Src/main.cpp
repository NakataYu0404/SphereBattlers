...
void DrawSpear(float x, float y, float angle, unsigned int color, float length) {
    float cos_a = cosf(angle);
    float sin_a = sinf(angle);
    
    // Spear shaft - use the actual weapon length
    ...
    float baseY1 = ey + cos_a * 4.0f * headScale;
    float baseX2 = ex + sin_a * 4.0f * headScale;
    float baseY2 = ey - cos_a * 4.0f * headScale;
        ...
        if (currentScene == SCENE_TITLE) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (circles[i].weapon.type == WEAPON_BOOMERANG) {
                    DrawBoomerang(weaponWorldX, weaponWorldY, circles[i].angle, circles[i].weapon.color, circles[i].weapon.length);
                } else {
                    DrawSpear(weaponWorldX, weaponWorldY, circles[i].angle, circles[i].weapon.color, circles[i].weapon.length);
                }
            }
